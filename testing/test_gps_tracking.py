...
def run(gps_enabled, accel_mag=1.0, bias_error=0.0, roll_tilt_deg=0.0, num_steps=400, dt=0.01, gps_period=0.2):
...
if __name__ == "__main__":
    ...
    for label, accel_mag, bias_error, tilt in scenarios:
        rms_pos_with, rms_vel_with, log_with = run(gps_enabled=True, accel_mag=accel_mag, bias_error=bias_error, roll_tilt_deg=tilt)
        rms_pos_without, rms_vel_without, log_without = run(gps_enabled=False, accel_mag=accel_mag, bias_error=bias_error, roll_tilt_deg=tilt)
